...
JRR_SET_CLASSPATH_AS_ARG=1
JRR_SET_AGENT_AS_ARG=1
JRR_CLASSPATH_ARG3=
...
JRR_JMX_OPTS="-Dcom.sun.management.jmxremote=true -Dcom.sun.management.jmxremote.ssl=false -Dcom_sun_management_jmxremote_authenticate=false"
JRR_JAVASTDFORCE=0
JRR_JAVAW=
JRR_ERROR_LEVEL=

# custom unix props
JRR_GROOVY_FILE1=${GR_HOME}/firstdownload/groovyrunner.groovy
JRR_ORIGINAL_ARGS=$@
...
jrr_set_my_java_ver_and_home(){
    if [  -d "$2" ] ; then
       ...
       # echo "jrr_set_my_java_ver_and_home : wrong arg : $*"
       if [ "$JRR_PRINT_JAVA_HOME_NOTFOUND" = 1 ]; then
...
#      JRR_JAVA8_USED=0    
  ...
  if [ "$JRR_ADD_JMX_UNSAFE" = 1 ]; then
     JRR_JVM_OPTS="$JRR_JVM_OPTS $JRR_JMX_OPTS"
  fi  
  ...
  if [ "$JRR_SET_CLASSPATH_AS_ARG" = 1 ]; then
     JRR_CLASSPATH_ARG3="-classpath $JRR_CLASSPATH"
  else
     export CLASSPATH=$JRR_CLASSPATH
  fi  
  
  
  if [ "$JRR_SET_AGENT_AS_ARG" = "0" ]; then
     export _JAVA_OPTIONS=$JRR_ALWAYS_OPTS
     JRR_ALWAYS_OPTS=
  fi 
  ...
  if [ $JRR_FORK_CMD = 1 ]; then
    $JRR_PREFIX_CMD $JRR_JAVA_PATH $JRR_FIRST_JVM_OPTS $JRR_MEMORY_XMX $JRR_JVM_OPTS $JAVA_OPTS $JRR_ALWAYS_OPTS $GROOVY_OPTS $JRR_CLASSPATH_ARG3 groovy.ui.GroovyMain $JRR_GROOVY_FILE1 $JRR_JAVATYPE $JRR_FULL_CMD $JRR_PROGA_ARGS_LAST &
    JRR_FORKED_PROCESS_PID=$!
