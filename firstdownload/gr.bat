...
set JRR_SET_CLASSPATH_AS_ARG=1
set JRR_SET_AGENT_AS_ARG=1
set JRR_CLASSPATH_ARG3=
...
set JRR_JMX_OPTS= -Dcom.sun.management.jmxremote=true -Dcom.sun.management.jmxremote.ssl=false -Dcom_sun_management_jmxremote_authenticate=false
set JRR_JAVASTDFORCE=0
set JRR_JAVAW=
set JRR_ERROR_LEVEL=

rem custom windows props
set JRR_GROOVY_FILE1=%GR_HOME%/firstdownload/groovyrunner.groovy
...
if "%JRR_ADD_JMX_UNSAFE%"=="1" (
  set JRR_JVM_OPTS=%JRR_JVM_OPTS% %JRR_JMX_OPTS%
)
...
if "%JRR_SET_CLASSPATH_AS_ARG%"=="1" (
   set JRR_CLASSPATH_ARG3=-classpath %JRR_CLASSPATH%   
)else (
   set CLASSPATH=%JRR_CLASSPATH%
)

if "%JRR_SET_AGENT_AS_ARG%"=="0" (
   set _JAVA_OPTIONS=%JRR_ALWAYS_OPTS%
   set JRR_ALWAYS_OPTS=
)
...
%JRR_PREFIX_CMD% "%JRR_JAVA_PATH%%JRR_JAVAW%" %JRR_FIRST_JVM_OPTS% %JRR_MEMORY_XMX% %JRR_JVM_OPTS% %JAVA_OPTS% %JRR_ALWAYS_OPTS% %GROOVY_OPTS% %JRR_CLASSPATH_ARG3% groovy.ui.GroovyMain %JRR_GROOVY_FILE1% %JRR_JAVATYPE% %JRR_FULL_CMD% %JRR_PROGA_ARGS_LAST%
