...
class RunnableFactory {
    ...
    static void runRunnerC(File file, Object callerInfo) {
        if (callerInfo == null) {
            throw new NullPointerException("caller info is null for ${file}")
        }
        assert file.exists()
        RunnableFile runner = createRunner(file)
        runner.callerInfo = callerInfo
        runner.run()
    }
    ...
    static void runRunnerC(Class clazz, Object callerInfo) {
        if (callerInfo == null) {
            throw new NullPointerException("caller info is null for ${clazz}")
        }
        RunnableClassName2 r = new RunnableClassName2(clazz)
        r.callerInfo = callerInfo
        r.run()
    }
    ...
    static void runRunnerC(String className, Object callerInfo) {
        runRunnerC(new ClRef(className), callerInfo)
    }
    ...
    static void runRunnerC(ClRefRef className, Object callerInfo) {
        if (callerInfo == null) {
            throw new NullPointerException("caller info is null for ${className}")
        }
        if (className == null) {
            throw new NullPointerException('class name is null')
        }
        RunnableClassName clazz = createRunnerFromClass(className.getClRef(), GroovyClassLoaderDefault.thisClassLoader)
        clazz.callerInfo = callerInfo
        clazz.run()
    }
