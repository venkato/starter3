...
class JrrStarterVariables2 {
    ...
    File detectJrrConfig2Dir() {
        ...
        if (config2DirL2.exists()) {
            debugFlags.add 13;
            File f = new File(config2DirL2.text.trim())
            if (!f.exists()) {
                println "${getClass().getName()} : config dir not exist 1 : ${f}"
            }
            return f
        }
        ...
        if (homeFromWindows != null) {
            ...
            if (config2DirL3.exists()) {
                debugFlags.add 14;
                File f = new File(config2DirL3.text.trim())
                if (!f.exists()) {
                    println "${getClass().getName()} : config dir not exist 2 : ${f}"
                }
                return f;
            }
        }
