...
//        gmrp.setPhaseChanger(JrrRunnerPhase2.,this.&)
    ...
    void loadUserConfig2Enriched() {
        if (JrrStarterVariables.filesDir!=null) {
            File config2File = new File(JrrStarterVariables.filesDir,JrrStarterConstatnts.configFileName)
...
//                try {
    ...
    void runTargetMethod() {
        try {
            ...
            if (gmrp2.runMainJavaMethod) {
                JrrClassUtils.runMainMethod(gmrp2.mainClassFound, args2.toArray(new String[0]))
            } else {
