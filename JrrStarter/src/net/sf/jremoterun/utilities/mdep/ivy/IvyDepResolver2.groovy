...
//        if(packageId==sourcesS) {
        //MDArtifact artifact = new MDArtifact(md, md.getModuleRevisionId().getName(), "source", "jar", null, Collections.singletonMap("m:classifier", "sources"));
//            ivySettings.getResolver(md.getModuleRevisionId()).locate(artifact)
//            md.addArtifact("sources",artifact)
//        }
        ResolveReport rr = ivy.resolve(md, ro);
        if (rr.hasError()) {
